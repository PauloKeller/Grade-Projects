...
for txta in textoa:
    if txta[0] in zum and txta[-1] not in zum:
        cont += 1

cont = 0

for txtb in textob:
    if txtb[0] in zum and txtb[-1] not in zum:
        cont += 1
        
print('Preposições texto B:',cont)
...
for txta in textoa:
    ...
    if len(txta) == 7:
      ...
      if txta[-1] not in zum:
          
        cont += 1
        
        if txta[0] not in zum:
...
for txtb in textob:
    ...
    if len(txtb) == 7:
        ...
        if txtb[-1] not in zum:
            
            cont += 1
            
            if txtb[0] not in zum:
...
def b20(num):
    ...
    n = 0
    for c in num:
        n += alfabeto.find(c)*p20
        p20 *= 20
    return n
teste = 'zmbzmb'

textoa = list(set(textoa))
dstA = []
cont = 0
print ('E)', end = ' ')
for x in textoa:
